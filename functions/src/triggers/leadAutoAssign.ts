...
interface SalesRepWithDistance {
  uid: string;
  displayName: string;
  distance: number;
 ...
 */
async function findClosestSalesRep(
  leadLat: number,
  leadLng: number
): Promise<SalesRepWithDistance | null> {
  ...
  const salesRepsWithDistance: SalesRepWithDistance[] = [];
  ...
  functions.logger.info(
    `Closest sales rep is ${closest.displayName} (${closest.uid}) at ${closest.distance.toFixed(1)} miles`
  );
...
async function processAutoAssignment(
  ...
  functions.logger.info(
    `Auto-assigned lead ${leadId} to ${closestRep.displayName} (${closestRep.uid}), distance: ${closestRep.distance.toFixed(1)} mi`
  );
 ...
 */
export const onLeadCreatedAutoAssign = functions
  .runWith({
    timeoutSeconds: 60,
    memory: '256MB',
  })
  ...
  .onCreate(async (snapshot, context) => {
    ...
    const leadAddress = leadData.customer?.address;
    let leadLat = leadAddress?.lat;
    let leadLng = leadAddress?.lng;
    ...
    if (!leadLat || !leadLng) {
      if (leadAddress?.zip) {
        ...
        const coords = await geocodeZipCode(leadAddress.zip);
        if (coords) {
          leadLat = coords.lat;
          leadLng = coords.lng;
 ...
 */
export const onUserGeocode = functions
  .runWith({
    timeoutSeconds: 30,
    memory: '256MB',
  })
  ...
  .onWrite(async (change, context) => {
    ...
    if (!needsGeocoding) return null;

    functions.logger.info(`Geocoding zip code ${userData.baseZipCode} for user ${userId}`);

    const coords = await geocodeZipCode(userData.baseZipCode);
